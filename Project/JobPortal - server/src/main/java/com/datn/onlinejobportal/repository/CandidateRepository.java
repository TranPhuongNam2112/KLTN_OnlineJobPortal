...
public interface CandidateRepository extends JpaRepository<Candidate, Long> {
	...
	@Query("Select new com.datn.onlinejobportal.dto.CandidateSummary(c.id, f.data, u.name, c.city_province, c.work_title, c.updatedAt, u.imageUrl) From Candidate c "
			...
			+ "OR i IN (SELECT i FROM Industry i Where i.industryname = ?4) OR j IN (SELECT j FROM JobType j Where j.job_type_name = ?5)  "
			+ "OR lower(c.city_province) LIKE lower(concat('%', ?6, '%'))")
